...
|   |                    | Basket is full     | Displays `"Basket is full"`|
| 2 | `Remove(string bagel)` | `bagel` exists in `Basket.Bagels`         | Removes `bagel` from `Basket.Bagels`|
|   |                       | `bagel` does not exist in `Basket.Bagels` | Displays `$"{bagel} was not found in basket"`|
| 3 | `ChangeCapacity(int capacity)` |  | Sets `Basket.Capacity` to `capacity`|